...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'grasswalk':
				dialogue = CoolUtil.coolTextFile(Paths.txt('grasswalk/grasswalkDialogue'));
			case 'crazy-dave':
				dialogue = CoolUtil.coolTextFile(Paths.txt('crazy-dave/crazy-daveDialogue'));
			case 'zombies-on-your-lawn':
				dialogue = CoolUtil.coolTextFile(Paths.txt('zombies-on-your-lawn/zombies-on-your-lawnDialogue'));
			case 'wabby-wabbo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('wabby-wabbo/wabby-wabboDialogue'));
			case 'senpai':
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'zombies-on-your-lawn':

				{
					...
					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('Zombies/MainNoMower'));
					...
					var zombiedancin:FlxSprite = new FlxSprite(-250, -80);
					zombiedancin.frames = Paths.getSparrowAtlas('Zombies/BackgroundZombies', 'week1');
					zombiedancin.animation.addByPrefix('zdancin', 'ZombieBackgroundDancin', 24, true);
					zombiedancin.antialiasing = true;
					zombiedancin.animation.play('zdancin');
					zombiedancin.scrollFactor.set(0.85, 0.85);
					add(zombiedancin);
	...
	function Darkness(?dialogueBox:DialogueBox):Void
	{
		add(black);
		if (dialogueBox != null)
							{
	...
	function startSong():Void
	{
		...
		switch(curSong)
		{
			case 'Crazy-Dave' | 'Philly' | 'Blammed' | 'Cocoa' | 'Eggnog': allowedToHeadbang = true;
	...
	*/
	...
	override function stepHit()
	{
		...
		//CRAZY DAVE STEPS PLIZ IGNORE THE NUMBERS, AM NOT GOOD CODER
		if (curSong == 'crazy-dave')
		{
			switch (curStep)
			{
					case 28, 92, 188, 220, 284, 348, 380, 412, 476, 508, 524, 540, 556, 572, 588, 604 :
					gf.playAnim('cheer', true);
					case 60, 124, 156, 252, 316, 444, 636 :
					boyfriend.playAnim('hey', true);
					gf.playAnim('cheer', true);
			}
		...
		//ZOMBIES ON YOUR LAWN STEPS
		if (curSong == 'zombies-on-your-lawn')
		{
			switch (curStep)
			{		
					case 128, 130, 132, 134, 136 ,138, 140, 142 :
					...
					case 1169:
					remove(boyfriend);
					boyfriend = new Boyfriend(770, 450,'bfwink');
					add(boyfriend);
					camZooming = false;
	...
	override function beatHit()
	{
		...
		if (curBeat % 8 == 7 && curSong == 'Crazy-Dave')
		{
			boyfriend.playAnim('hey', true);
		}
